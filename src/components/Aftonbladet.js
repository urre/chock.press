...
import AftonbladetLogo from './AftonbladetLogo'
...
class Aftonbladet extends Component {
	...
	getArticles() {
		...
		axios
			...
			.then(() => {
				document
					.querySelector('.articles--aftonbladet')
				...
				for (let i = 0; i < words.length; i++) {
					if (el.innerHTML.indexOf(words[i])) {
						const instance = new Mark(
							document.querySelector('.articles--aftonbladet')
						)
		...
		return this.state.articles.map((article, index) => {
			return (
				<li key={index}>
					<a
						href={
							article.url.includes('aftonbladet')
								? article.url
								: `http://aftonbladet.se${article.url}`
						}
					>
						{article.title}
					</a>
