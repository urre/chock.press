...
class Expressen extends Component {
	...
	getArticles() {
		...
		axios
			...
			.then(() => {
				document
					.querySelector('.articles--expressen')
	...
	markWords(title) {
		axios.get('../../data/words.json').then(function(res) {
			...
			const elements = document.querySelectorAll('.articles--expressen a')
			Array.prototype.forEach.call(elements, function(el, i) {
				for (let i = 0; i < words.length; i++) {
					if (el.innerHTML.indexOf(words[i])) {
						const instance = new Mark(
							document.querySelector('.articles--expressen')
						)
		...
		return this.state.articles.map((article, index) => {
			return (
				<li key={index}>
					<a
						href={
							article.url.includes('expressen')
								? article.url
								: `http://expressen.se${article.url}`
						}
					>
						{article.title}
					</a>
